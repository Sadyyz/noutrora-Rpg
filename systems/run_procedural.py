...
def _evento_inscricoes(jogador):
    ...
    print("\n  Paredes cobertas de inscrições em uma língua que dói nos olhos.")
    time.sleep(1)
    print("  Você olha com cuidado. Algumas parecem... familiares.")
    time.sleep(1)

    if runs > 1:
        print(f"\n  Uma das inscrições tem seu nome. E datas.")
        time.sleep(1)
        print(f"  Ao lado: marcas de morte. {mortes} delas.")
        time.sleep(1)
        print("  Esta masmorra te conhece.")
        memoria.registrar_evento(f"{jogador.nome} viu seu nome inscrito na parede.")
    else:
        print("\n  Uma inscrição se destaca: 'O primeiro sempre esquece o caminho.'")
        time.sleep(1)
        print("  Você não entende agora. Mas vai entender.")
